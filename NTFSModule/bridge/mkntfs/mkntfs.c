 ...
 */
struct ntfs_device *nk_mkntfs_external_dev = NULL;
/* Set once mkntfs_redirect takes ownership of the device (its cleanup then
 * frees it). If a run fails before this, the caller must free the device. */
int nk_mkntfs_dev_consumed = 0;
...
static void mkntfs_cleanup(void)
{
	...
	/* NTFSKit patch: make a second in-process run safe — the original tool
	 * exits after one run and never resets these. */
	g_allocation = NULL;
	g_num_bad_blocks = 0;
	free(g_upcaseinfo);
	g_upcaseinfo = NULL;
...
static BOOL mkntfs_open_partition(ntfs_volume *vol)
{
	 ...
	 * the volume.
	 */
	vol->dev = nk_mkntfs_external_dev ? nk_mkntfs_external_dev :
		ntfs_device_alloc(opts.dev_name, 0, &ntfs_device_default_io_ops, NULL);
	if (nk_mkntfs_external_dev)
		nk_mkntfs_dev_consumed = 1;
	if (!vol->dev) {
 ...
 */
int nk_mkntfs_main(int argc, char *argv[])
{
	int result = 1;
